...
export const StudioInlineInsertBar: React.FC<StudioInlineInsertBarProps> = ({
    ...
    return (
        <div className={`relative h-0 my-0 flex items-center justify-center transition-all ${
            isOpen ? 'z-40 opacity-100' : 'group/insert opacity-0 hover:opacity-100 z-20'
        }`}>
            {/* Thin divider line appearing on hover */}
            <div className={`absolute inset-x-2 top-1/2 -translate-y-1/2 h-[1px] pointer-events-none ${
                isDarkTheme ? 'bg-purple-500/50' : 'bg-purple-400'
            }`} />

            {/* Floating small round (+) button directly on the border seam */}
            <button
                type="button"
                onClick={onToggle}
                className={`relative z-30 w-5 h-5 -translate-y-1/2 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm transition-transform hover:scale-125 ${
            ...
            {isOpen && (
                <div className={`absolute top-4 z-50 border rounded-xl p-2 shadow-xl flex flex-wrap gap-1.5 max-w-md justify-center animate-in fade-in zoom-in-95 duration-150 ${
                    ...
                    <button
                        ...
                        className={`px-2 py-1 border rounded text-xs font-semibold flex items-center gap-1 transition-all ${

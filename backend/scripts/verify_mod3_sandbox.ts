...
async function verifyMod3() {
  ...
  console.log(`=== BẮT ĐẦU CHẤM THỬ TOÀN BỘ BÀI TẬP MODULE 3 VỚI SANDBOX ===`);
  let passedCount = 0;
  let failedCount = 0;
  const failures: any[] = [];
  ...
  console.log(`\n======================================================`);
  console.log(`KẾT QUẢ MODULE 3: ${passedCount} PASSED / ${failedCount} FAILED`);
  console.log(`======================================================`);

  if (failures.length > 0) {
    console.log(`\nCHI TIẾT CÁC BÀI FAILED:`);
    for (const f of failures) {
      console.log(`\n- Bài: "${f.title}" (${f.lesson})`);

...
- **ADR-014**: **Monolith Backend + SPA Frontend + FastAPI AI Microservice.** App is a Node/Express backend, React/Vite frontend, and FastAPI recommendation service (`ai-service/main.py`).
...
- **Pattern**: `ERROR: Could not import module "main"` when starting `uvicorn`.
  - *Fix*: Create `ai-service/main.py` entrypoint with FastAPI app.

## 3. Session Snapshots
- **Snapshot 2026-08-04T09:00 (git `99e1757`, dirty)**: Ran `/init` (Scenario B) on LearnPython codebase. Hydrated `ARCHITECTURE.md`, `FEATURES.md`, `hot_memory.json`, appended ADRs 010-014.
- **Snapshot 2026-08-05T21:15**: Configured Supabase DB env, fixed seed `P2002` error, seeded database successfully, scaffolded `ai-service/main.py` with FastAPI endpoints, and updated `hot_memory.json` & `cold_memory.md`.
